...
while current_time + interval_duration <= end_time:
    ...
    if n % long_break_interval == 0:
        ...
        schedule += create_event(f"{long_break_interval}min Break", current_time.strftime("%Y%m%dT%H%M%S"), (current_time + long_break_duration).strftime("%Y%m%dT%H%M%S"))
        current_time += long_break_duration
    else:
        # Create the short break event
        schedule += create_event(f"{short_break_duration}min Break", current_time.strftime("%Y%m%dT%H%M%S"), (current_time + short_break_duration).strftime("%Y%m%dT%H%M%S"))
